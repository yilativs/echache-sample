...
public class HazelcastComparison {
	...
	private static void compareInMemoryStrategies() {
		...
		long startSerializedHazelcastTime = System.currentTimeMillis();
		try (HazelcastCacheAsideUserService hazelcastUserService = new HazelcastCacheAsideUserService()) {
			for (Integer id : randomIdArray) {
				// long interationStartTime = System.currentTimeMillis();
				User user = hazelcastUserService.getUserFromMap(id);
				// long timeTaken = System.currentTimeMillis() - interationStartTime;
				// System.out.println("id=" + user.getId() + " time=" + timeTaken);
			}
			long totalSerializedHazelcastTimeTaken = System.currentTimeMillis() - startSerializedHazelcastTime;
			System.out.println("totalSerializedHazelcastTimeTaken=" + totalSerializedHazelcastTimeTaken);
		}
